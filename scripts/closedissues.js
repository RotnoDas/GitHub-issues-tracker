...
const closedIssuesCounter = () => {
    const closedIssuesCount = document.getElementById('closed-issue-count');
    closedIssuesCount.innerText = closedIssues.length;
}
...
const displayClosedIssues = (closedIssues) => {
    ...
    closedIssues.forEach((issue) => {
        ...
        `;
        closedIssuesContainer.appendChild(closedDiv);
        closedIssuesCounter();
    })
    manageLoading(false);
};

const pushDataClosed = (issuesData) => {
    closedIssues.length = 0;
    issuesData.forEach((element) => {
        if(element.status === "closed") {
            closedIssues.push(element);
        }
    });
...
const loadClosedIssues = () => {
    ...
    fetch(url)
        ...
        .then((issuesData) => {
            pushDataClosed(issuesData.data);
        })
};
...
closedButton.addEventListener('click', () => {
    removeActiveButton();
    closedButton.classList.add("active-button");
    manageLoading(true);
